...
public class AdminDashboardController extends HttpServlet {
    ...
    public void init() throws ServletException {
        // Initialize only if not already injected (useful for testing with Mockito)
        if (productService == null) productService = ProductService.getInstance();
        if (customerService == null) customerService = CustomerService.getInstance();
        if (auditLogDAO == null) auditLogDAO = new AuditLogDAO();
        if (billDAO == null) billDAO = new BillDAO();
        if (customerDAO == null) customerDAO = new CustomerDAO();
    }

    // 👇 Setter methods for test injection
    ...
    private void showAuditLog(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = com.pahanaedu.dao.DBConnection.getConnection()) {
            List<AuditLog> auditLogs =
                    (search != null && !search.trim().isEmpty()) || (category != null && !category.isEmpty())
                            ? auditLogDAO.searchLogs(conn, search, category)
                            : auditLogDAO.getAllLogs(conn);
    ...
    private void showDashboard(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = com.pahanaedu.dao.DBConnection.getConnection()) {
            ...
            request.setAttribute("billsToday", 0); // placeholder
